...
private:
    ...
    TreeNode* insert(TreeNode* node, int code, const std::string& popCounty) {
        if (node == nullptr) {
            insertCount++; // Increment insert count
...
public:
    ...
    PopMap(const std::string& filename) : root(nullptr), searchCount(0), insertCount(0), eraseCount(0) {
    ...
    void find(int code) {
        ...
        std::cout << "Nodes examined in search: " << searchCount << std::endl;
    }
    ...
    void insert(int code, const std::string& popCounty) {
        ...
        std::cout << "Nodes examined in insert: " << insertCount << std::endl;
    }
    ...
    void erase(int code) {
        ...
        std::cout << "Nodes examined in erase: " << eraseCount << std::endl;
    }
    ...
    TreeNode* erase(TreeNode* node, int code) {
        ...
        eraseCount++; // Increment erase count
        return node;
    }
...
int main() {
    ...
    popDatabase.print(); 
    popDatabase.find(6037); 
    popDatabase.find(6000);
    popDatabase.insert(6066, "1, New County, CA"); 
    popDatabase.insert(6065, "2000, Riverside, CA"); 
    popDatabase.erase(6999); 
    popDatabase.erase(6075); 
    popDatabase.erase(6055); 
    popDatabase.print(); 
